...
function responsive_styles() {
	wp_enqueue_style('responsive-style', get_theme_file_uri( '/assets/css/responsive.css' ), array());
}
add_action( 'wp_head', 'responsive_styles' );

function zino_styles2() {
	wp_enqueue_style('zino-style2', get_theme_file_uri( '/assets/css/style.css' ), array());
}
add_action( 'wp_head', 'zino_styles2' );

/**
 * function.ファイルの追加
 */
function function_script() {
	wp_enqueue_script( 'function-js', get_theme_file_uri( '/assets/js/function.js' ), array());
}
add_action( 'wp_head', 'function_script' );
